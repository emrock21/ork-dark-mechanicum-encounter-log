...
/// @title Dark Mechanicum Encounter Log
/// @notice Records short Ork descriptions of encounters with Dark Mechanicum forges and daemon engines.
/// @dev Fully safe: no payments, no owner, no external calls, no selfdestruct.

contract DarkMechanicumEncounterLog {
    ...
    event EncounterRecorded(uint256 indexed id, string forge, address indexed creator);
    event EncounterVoted(uint256 indexed id, bool approved, uint256 approvedVotes, uint256 rejectedVotes);

    constructor() {
        logs.push(
            Encounter({
                forge: "Example Dark Forge (Fill your own below)",
                behavior: "Describe how da hereteks or daemon engines acted.",
    ...
    function recordEncounter(
        string calldata forge,
        string calldata behavior,
        string calldata outcome
    ) external {
        require(bytes(forge).length > 0, "Forge required");
        ...
        emit EncounterRecorded(logs.length - 1, forge, msg.sender);
    }
